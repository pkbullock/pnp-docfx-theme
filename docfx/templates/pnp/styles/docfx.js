...
$(function () {
  ...
  function renderNavbar() {
    var navbar = $('#navbar menu')[0];
    if (typeof (navbar) === 'undefined') {
      loadNavbar();
    } else {
      //$('#navbar menu a.active').parents('li').addClass(active);
      renderBreadcrumb();
      showSearch();
    }
    ...
    function loadNavbar() {
      ...
      $.get(navbarPath, function (data) {
        $(data).find("#toc>menu").appendTo("#navbar");
        showSearch();
        ...
        //$('#navbar>menu').addClass('navbar-nav');
        ...
        renderNavbar();
        setupHamburger(); // Stefen's JS code
      });
    }
  }
 ...
 * https://pkbullock.github.io/pnp-docfx-theme/
 */
  var baseAddress = (document.location.host === "pkbullock.github.io") ? document.location.origin + "/pnp-docfx-theme" : document.location.origin;
  ...
  function loadSample(sample, filter) {
    try {
      ...
      var modified = new Date(sample.updateDateTime).toDateString();
      ...
      if (authors.length < 1) {
        ...
        //});
        if(authors.length > 1){
          latestAuthor = latestAuthor + ` +${authors.length - 1} more`;
        }
      ...
      // Libraries
      var librariesName = "";
      var librariesFriendlyNameDict = {};
      librariesFriendlyNameDict["pnp-powershell"] = "PnP PowerShell";
      librariesFriendlyNameDict["cli-for-microsoft365"] = "CLI for Microsoft 365";
      librariesFriendlyNameDict["powershell"] = "PowerShell";

      $.each(sample.metadata, function (_u, library) {

        var key = library.key.toLowerCase();

        if (librariesFriendlyNameDict.hasOwnProperty(key)) {
          librariesName = librariesName + `<div class="blog-cat">` + librariesFriendlyNameDict[key] + `</div>`;
        } else {
          librariesName = librariesName + `<div class="blog-cat">` + library.key + `</div>`;
        }
      });
...
// </a>`);
      ...
      // Includes LI wrapper for no-isotope version
      var sampleItem = $(`<li>
      <a class="card-link" href="${sample.url}" data-modified="${sample.updateDateTime}" data-title="${title}" data-keywords="${keywords}" data-tags="${tags}" data-libraries="${libraries}" data-operation="${operations}" data-products="${products}">
        <article class="card sample" >
          <figure class="sample-media">
            <img src="${thumbnail}" class="sample-media-img" alt="${title}" />
          </figure>
          <div class="card-hor">
            <h2 class="sample-headline">${sample.title}</h2>
            <p class="sample-desc">${shortDescription}</p>
            <div class="blog-meta">
              <div class="blog-info">
                <time class="blog-created" datetime="${modified}">${modified}</time>
                <div class="blog-author">Updated by ${latestAuthor}</div>
              </div>
              <div class="blog-type">
                ${librariesName}
              </div>
...
// End of No Isotope ------------------------------------------------------------------
  ...
  // init Isotope
  var $grid = $('#sample-listing-disabled').isotope({
    itemSelector: '.card.sample',
    layoutMode: 'fitRows',
    sortBy: 'modified',
    sortAscending: false,
    getSortData: {
